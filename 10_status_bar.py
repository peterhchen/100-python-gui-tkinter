...
status = Label (root, text="Image 1 of " + str(len(image_list)), relief=SUNKEN, anchor=E)
status.grid(row=2, column=0, columnspan=3, sticky=W+E)  # Sticky West and EAST stretch
# SUNKEN display status bar drown-in style. anchor at the East side.
...
def forward(image_number):
    ...
    status = Label (root, text="Image " + str(image_number) + " of " \
        + str(len(image_list)), relief=SUNKEN, anchor=E)
    status.grid(row=2, column=0, columnspan=3, sticky=W+E)  # Sticky West and EAST stretch
    if image_number == 5:
